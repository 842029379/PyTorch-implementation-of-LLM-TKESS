...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    ...
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, _) in tqdm(enumerate(vali_loader)):
            batch_x = batch_x.float().to(device)
            batch_y = batch_y.float().to(device).unsqueeze(-1)

            batch_x_mark = batch_x_mark.float().to(device)
            outputs = model(batch_x, batch_x_mark, i)
...
def test(model, test_data, test_loader, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            preds.append(shift_outputs)
            trues.append(shift_labels)
